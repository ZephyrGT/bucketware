    ...
    public class World
    {
        ...
        private int Tile_Serialize(byte[] dataPassed, int loc)
        {
            try
            {
                if (readPos >= dataPassed.Length) {
                    Proxyhelper.LogText += ("[" + DateTime.UtcNow + "] (PROXY): [" + currentWorld + "] readPos bigger than dataPassed.Length error during world serialization!\n");
                    return -1;
                }
        ...
        private int WorldObjects_Serialize(byte[] dataPassed)
        {
            try
            {
                ...
                Proxyhelper.LogText += ("[" + DateTime.UtcNow + "] (PROXY): Serialized +1 dropped item object. ID: " + dropped.id.ToString() + " X:" + dropped.x.ToString() + " Y: " + dropped.y.ToString() + " UID: " + dropped.uid.ToString() + "\n");
            }
            catch (ArgumentException)
            {
                return -1;
        ...
        public World LoadMap(byte[] packet)
        {
            try
            {
                ...
                Proxyhelper.LogText += ("[" + DateTime.UtcNow + "] (PROXY): Loading world: " + currentWorld + ", total tile count is: " + tileCount.ToString() + "\n");
                if (!Proxyhelper.globalUserData.serializeWorldsAdvanced)
                {
                    Proxyhelper.LogText += ("[" + DateTime.UtcNow + "] (PROXY): NOTE: Advanced world serialization was disabled, not continuing to load tiles.\n");
                    return this;
                }
                ...
                Proxyhelper.LogText += ("[" + DateTime.UtcNow + "] (PROXY): [" + currentWorld + "]" + " Tiles properly serialized (without any errors): " + tilesProperlySerialized.ToString() + "\n");
                if (readPos >= data.Length)
                {
                    Proxyhelper.LogText += ("[" + DateTime.UtcNow + "] (PROXY): [" + currentWorld + "] readPos error during world serialization!\n");
                    return this;
                }
                ...
                Proxyhelper.LogText += ("[" + DateTime.UtcNow + "] (PROXY): Succeeded loading world '" + currentWorld + "'! Total bytes serialized: " + total_bytes_serialized.ToString() + ",  dropped item uid count: " + count + "\n");
            }
            catch
            {
                return this;
